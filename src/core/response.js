...
const SYMBOL_BODY = Symbol('Response#body')
...
module.exports = class Response {
  ...
  set body(val) {
    ...
    if (val instanceof Stream) {
      ...
      if (val.listeners('error').indexOf(this.onError) === -1) {
  ...
  get length() {
    ...
    if (len) return ~~len
    return getLength(this[SYMBOL_BODY], this.flag)
  }
  ...
  redirect(url, alt) {
    // location
    if ('back' === url) url = this.req.get('referrer') || alt || '/'
